...
const calcPath = (startLocation: Location, endLocation: Location): LocationWithValue[] => {
    ...
    while (true) {
        ...
        },
        {
            ...curLocation,
            y: curLocation.y + 1
        }].filter(filterLocation => filterLocation.y >= 0 && filterLocation.y < grid.length
            && filterLocation.x >= 0 && filterLocation.x < grid[filterLocation.y].length
            && grid[filterLocation.y][filterLocation.x].type !== 'wall'
            && visitedLocations.find(visitedLocation => visitedLocation.x === filterLocation.x && visitedLocation.y === filterLocation.y)?.value === undefined).map(location => ({...location, value: location.value + 1}));
...
const MazeComponent = () => {
    ...
    const realPath: LocationWithValue[] | null = useMemo(() => {
        const realPath = [];
        const startLocationWithValue = path.find(location => location.x === startLocation.x && location.y === startLocation.y);
        ...
        while (startLocationWithValue.value > 0) {
            ...
            const newLocation = path.find(location => location.value === (curValue - 1)
                && (Math.abs(location.x - curX) + Math.abs(location.y - curY)) === 1);
            if (!newLocation) {
                break;
            }
    ...
    return (
        <>
            {grid.map((row, rowIndex) => {
                return <Box key={rowIndex} display='flex' flexDirection='row' flexWrap='nowrap'>
                    {row.map((tile, colIndex) => {
                        return <Box key={colIndex} width='50px' height='50px' bgcolor={tile.type === 'wall' ? 'black' : 'grey'} border='1px solid black' position='relative'>
                            {(colIndex === startLocation.x && rowIndex === startLocation.y) &&
                            <Box width='100%' height='100%' display='flex' justifyContent='center' alignItems='center' position='absolute'>
                                <Typography>
                                    S
                                </Typography>
                            </Box>}
                            ...
                            <Box width='100%' height='100%' display='flex' justifyContent='center' alignItems='center' bgcolor='green' position='absolute'>
                                <Typography>
                            ...
                            <Box width='100%' height='100%' display='flex' justifyContent='center' alignItems='center' position='absolute'>
                                <Button style={{width: '100%', height: '100%'}} onClick={() => setStartLocation({
                                    x: colIndex,
                                    y: rowIndex
                                })}/>
                            </Box>
